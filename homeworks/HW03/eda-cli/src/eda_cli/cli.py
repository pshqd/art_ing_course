...
def report(
    ...
    title: str = typer.Option("Report", help="Название отчета"),
    top_k_cats: int = typer.Option(5, help="Сколько категорий оставлять в топе категорий.")
    ...
    top_cats = top_categories(df, top_k=top_k_cats)

    # 2. Качество в целом
    quality_flags = compute_quality_flags(summary, df,missing_df)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- [!] Есть ли константный столбец: **{quality_flags['has_constant_columns']}**\n")
        f.write(f"- [!] Дублируются ли user_id: **{quality_flags['has_suspicious_id_duplicates']}**\n")
