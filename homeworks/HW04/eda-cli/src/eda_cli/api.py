...
class DataFrameResponse(BaseModel):
    """Ответ - информация о датасете."""
    n_columns: int = Field(...,description = 'Количество колонок'),
    message: str = Field(
        ...,
        description="Человекочитаемое пояснение решения",
    )
...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    flags_all = compute_quality_flags(summary, df, missing_df)
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    """
    Эндпоинт, который принимает CSV-файл compute_quality_flags
    и возвращает оценку качества данных по флагам.

    Используются добавленные в прошлом дз оценки качества.
    """
    start = perf_counter()
